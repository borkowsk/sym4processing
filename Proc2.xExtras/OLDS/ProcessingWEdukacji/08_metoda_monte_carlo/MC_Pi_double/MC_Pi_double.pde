...
{ //Ile realnie punktów na sekundę i przy jakiej liczbie klatek
  ...
  s = 4. * ( (double)nk/(double)n );  //Ostateczne obliczenie proporcji
...
void draw() //Jest wykonywane w niewidocznej, nieskończonej pętli
{
  ...
  {
    double x = Math.random() * 2. - 1.;//Java Math.random()
    double y = Math.random() * 2. - 1.;//returns a double from zero to one.
    n++; //kolejna próba 
    ...
    if(x*x + y*y <= 1.0) //równanie okregu liczone na doublach
    {
        nk++;          //punkt wewnątrz
    ...
    point((float)x*width,(float)y*height);//Point oczekuje parametru typu float
  }
}
