...
void draw() //Jest wykonywane w niewidocznej pętli
{
  ...
  //Właściwy model - zmiana preferowanego ruchu raz na sekundę
  if(licz++ % FR == 0) // raz na sekundę
  {
    ah+=random(-50.0,50.0);//Sterowanie wg. preferencji
    ax+=random(-50.0,50.0);//może być asymetryczne - można zobaczyć do czego to prowadzi
    //println(ax,' ',ah); //Lubi utknąć przy jakiejś ściance lub w rogu
  }
