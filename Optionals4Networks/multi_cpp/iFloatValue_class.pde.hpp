/// @file
#pragma once
/// @date 2024-10-21 (last modification)                       @author borkowsk
#ifndef HEADER_iFloatValue_INCLUDED
#define HEADER_iFloatValue_INCLUDED

/** @brief Interface forces getter for single float. */

/// @note Automatically extracted definition of `class`: iFloatValue
//interface
class iFloatValue: public virtual Object{
  public:

  virtual  float                get() =0;
} ;//_EofCl

/// Generated by Processing2C++ extraction Tools
#endif //HEADER_iFloatValue_INCLUDED

//MADE NOTE: /data/wb/SCC/public/Processing2C/scripts did it 2024-10-21 19:06:46 !

