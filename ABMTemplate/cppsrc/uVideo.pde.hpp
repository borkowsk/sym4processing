...
///
///  initVideoExport(SAFE_THIS,FileName,Frames)); // The VideoExport class must have access to
...
static bool        videoExportEnabled=false; ///< Has film making been initiated?
...
{ /*_OnlyProcessingBlockBegin
  ...
  { 
   ...
   for(int i=0;i<videoFramesFreq;i++) //Have to last about one second
       videoExport->saveFrame(); //Video frames for const freeze
