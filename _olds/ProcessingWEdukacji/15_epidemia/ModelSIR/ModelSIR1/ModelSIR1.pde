...
{  
  for(int i=0;i<World.length;i++)//Wizualizacja czyli "rysowanie na ekranie" 
    for(int j=0;j<World.length;j++) 
    {
      switch(World[i][j]){ //Instrukcja wyboru pozwala nam wybrać 
      case 3:stroke(0,255,0);break;//dowolny kolor w zależności od liczby w komórce
  ...
  for(int a=0;a<World.length*World.length;a++)//Tyle losowań ile komórek
  {
       ...
       int neigh=(int)random(4);//który sąsiad
