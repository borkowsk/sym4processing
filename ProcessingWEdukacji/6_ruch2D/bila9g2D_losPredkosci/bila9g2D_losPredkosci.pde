...
void draw() //Jest wykonywane w niewidocznej pętli
{
  ...
  //Właściwy model
  if(licz++ % FR == 0) // raz na sekundę zmienia prędkość
  {
    vh=random(-150.0,150.0);//Sterowanie wg. preferencji
    vx=random(-150.0,150.0);//może być asymetryczne - można zobaczyć do czego to prowadzi
    println(vx,' ',vh);
  }
   //<>//
