...
float M=0.90; //Wydajność odbicia sprężystego 1-B = ile energi kinetycznej się rozprasza nie wraca do prędkości po odbiciu
...
void draw() //Jest wykonywane w niewidocznej pętli
{
  ...
  R+=random(-2.0,2.0);
  G+=random(-2.0,2.0);
  B+=random(-2.0,2.0);//Losuje z zakresu i o tyle powiększa (jak ujemna to pomniejsza)
  //print(R,' ');//,G,' ',B,"; ");
  fill(R,G,B);
  stroke(R/2,G/2,B/2);
  ellipse(x,height-h,25,25);
  
  //Jak się "intencjonalnie" zmieniają przyśpieszenia?
  //
  // ... TU UZUPEŁNIĆ ...
  //
